...
def get_all_employees():
    ...
    try:
        print('🔍 GET /employees isteği alındı')
        employees = qdrant_service.list_employees()
        print(f'🔍 Qdrant\'dan {len(employees)} çalışan alındı')
        ...
        print(f'🔍 Response hazırlandı: {len(employees)} çalışan')
        return jsonify(response_data), 200
        
    except Exception as e:
        print(f'❌ GET /employees hatası: {e}')
...
def add_employee():
    ...
    try:
        ...
        employee_data = employee_request.model_dump()  # Pydantic v2 için model_dump()
